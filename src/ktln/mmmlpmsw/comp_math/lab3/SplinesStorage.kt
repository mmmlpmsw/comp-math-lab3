...
import ktln.mmmlpmsw.comp_math.lab3.Spline

class SplinesStorage(x: DoubleArray, y: DoubleArray) {
    private lateinit var splines: Array<Spline?>
    private fun initSplines(x: DoubleArray, y: DoubleArray) {
        val n = x.size
    ...
    fun getInterpolatedY(x: Double): Double {
        val spline: Spline?
        if (x >= splines[splines.size - 1]!!.x) spline = splines[splines.size - 1] else if (x <= splines[0]!!.x) spline = splines[0] else {
            var k: Int
            var left = 0
